...
class HealthKitAccess {
    ...
    func getHRVData(dateRange: (from: Date, to:Date)?, completion: @escaping ([(date: Date, value: Double)]) -> Swift.Void){
        getQuantityData(quantityType: HKSampleType.quantityType(forIdentifier: .heartRateVariabilitySDNN)!, dateRange: dateRange) { (samples) in
            var result: [(date: Date, value: Double)] = []
            for s in samples{
                if let qs = s as? HKQuantitySample{
                    result.append((s.startDate, qs.quantity.doubleValue(for: HKUnit(from: "ms"))))
                }
            }
            completion(result)
        }
